<aside
    ...
    <div class="border-b border-white/10 px-6 py-6">
        ...
        <p class="mt-1 text-sm text-slate-400">Panel Pengguna</p>
    </div>
    ...
    <nav class="flex-1 space-y-2 overflow-y-auto px-4 py-6">
        ...
        <a href="{{ route('user.dashboard') }}"
            class="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition {{ request()->routeIs('user.dashboard') ? $active : $inactive }}">
        ...
        <!-- Data Keluarga -->
        <a href="{{ route('user.kartu-keluarga.index') }}"
            class="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition {{ request()->routeIs('user.kartu-keluarga.*') ? $active : $inactive }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
        ...
        <a href="{{ route('user.pengajuan-surat.index') }}"
            class="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition {{ request()->routeIs('user.pengajuan-surat.*') ? $active : $inactive }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
        ...
        <a href="{{ route('user.bantuan.index') }}"
            class="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition {{ request()->routeIs('user.bantuan.*') ? $active : $inactive }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
